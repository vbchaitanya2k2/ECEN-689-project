...
# One unified mutator. The DIFUZZ_SCHED env var picks the scheduling variant:
#   baseline -> uniform random.choice, FIFO eviction (paper/artifact default)
#   pow      -> total-reward weighted pick, FIFO eviction
#   powrate  -> smoothed success-rate weighted pick, FIFO eviction
#   evict    -> smoothed success-rate weighted pick + smart eviction
# Defaults to 'evict' (current best). Unknown values fall back to 'baseline'.
_SCHED = os.environ.get('DIFUZZ_SCHED', 'evict').strip().lower()
if _SCHED not in ('baseline', 'pow', 'powrate', 'evict'):
    print('[DifuzzRTL] Unknown DIFUZZ_SCHED={!r}, falling back to baseline'.format(_SCHED))
    _SCHED = 'baseline'
print('[DifuzzRTL] DIFUZZ_SCHED = {}'.format(_SCHED))
...
class rvMutator():
    ...
    def score_seed_rate(self, success, pick_count):
        # Laplace-smoothed success rate: neutral 0.5 for unknown seeds, bounded by data
        return (success + 1.0) / (pick_count + 2.0)

    def pick_corpus_idx(self, track_parent=True):
        if _SCHED == 'baseline':
            idx = random.randrange(len(self.corpus))
            if track_parent:
                self.last_picked_idx = idx
            return idx

        idx = random.choices(range(len(self.corpus)),
                             weights=self.corpus_score, k=1)[0]

        if track_parent:
            self.last_picked_idx = idx
            self.corpus_pick_count[idx] += 1
            if _SCHED in ('powrate', 'evict'):
                self.corpus_score[idx] = self.score_seed_rate(
                    self.corpus_success[idx], self.corpus_pick_count[idx])
    ...
    def get(self, assert_intr=False):
        ...
        if self.phase == GENERATION:
            ...
            if self.phase == MUTATION:
                idx = self.pick_corpus_idx()
                seed_si = self.corpus[idx]
                ...
                seed_words = []
                idx1 = self.pick_corpus_idx()
                idx2 = self.pick_corpus_idx(False)
                seed_si1 = self.corpus[idx1]
                seed_si2 = self.corpus[idx2]
    ...
    def add_corpus(self, sim_input):
        ...
        self.corpus_success.append(0)
        self.corpus_pick_count.append(0)
        if _SCHED == 'pow':
            # total-reward scheme: start at 1.0, add 1.0 per successful pick
            self.corpus_score.append(1.0)
        elif _SCHED in ('powrate', 'evict'):
            self.corpus_score.append(self.score_seed_rate(0, 0))
        else:
            # baseline: scores unused but keep arrays aligned so indexing is safe
            self.corpus_score.append(1.0)

        self.num_words = min(self.num_words + 1, self.max_nWords)
        if len(self.corpus) > self.corpus_size:
            if _SCHED == 'evict':
                evict = self._pick_evict_idx()
            else:
                evict = 0  # FIFO
            self.corpus.pop(evict)
            self.corpus_score.pop(evict)
            self.corpus_success.pop(evict)
            self.corpus_pick_count.pop(evict)
            if self.last_picked_idx is not None:
                if self.last_picked_idx == evict:
                    self.last_picked_idx = None
                elif self.last_picked_idx > evict:
                    self.last_picked_idx -= 1

    def _pick_evict_idx(self, min_picks=5):
        # Drop the oldest seed that has been tried >= min_picks times with zero
        # successes. Fall back to FIFO (index 0) when no proven dud exists.
        for i in range(len(self.corpus)):
            if self.corpus_pick_count[i] >= min_picks and \
               self.corpus_success[i] == 0:
                return i
        return 0

    def reward_parent(self):
        if self.last_picked_idx is None:
            return
        idx = self.last_picked_idx
        if idx >= len(self.corpus_score):
            return
        if _SCHED == 'baseline':
            return  # no reward tracking in baseline
        self.corpus_success[idx] += 1
        if _SCHED == 'pow':
            self.corpus_score[idx] += 1.0  # total-reward bump
        else:  # powrate, evict
            self.corpus_score[idx] = self.score_seed_rate(
                self.corpus_success[idx], self.corpus_pick_count[idx])
