...
class rvMutator():
    ...
    def add_data(self, new_data=[]):
        ...
        if new_data:
            ...
            self.random_data[seed] = [random.randint(0, 0xffffffffffffffff) for i in range(
                64 * 6)]  # TODO, Num_data_sections = 6
    ...
    def make_nop(self, sim_input, nop_mask, part):
        ...
        if part == PREFIX:
            min_input = simInput(new_target, words, suffix,
                                 ints, data_seed, template)
        elif part == MAIN:
            new_ints = []
            k = 0
            for i in range(len(nop_mask)):
                if nop_mask[i]:
                    ...
                    new_ints += [ints[k + j]
                                 for j in range(new_target[i].len_insts)]

                k += new_target[i].len_insts

            min_input = simInput(prefix, new_target, suffix,
                                 new_ints, data_seed, template)
        else:
            min_input = simInput(prefix, words, new_target,
                                 ints, data_seed, template)
    ...
    def delete_nop(self, sim_input):
        ...
        del_input = simInput(
            words_map[PREFIX], words_map[MAIN], words_map[SUFFIX], new_ints, data_seed, template)
        data = self.random_data[data_seed]
